...
// (221:18) {#each Array(numDots) as _}
...
// (208:10) {#each week as { dayOfMonth, date, formattedDate, numDots }}
...
async function createDailyNote(date, dailyNoteSettings) {
    ...
    if (template) {
        try {
            const templateFile = vault.getAbstractFileByPath(normalize(template));
            templateContents = await vault.cachedRead(templateFile);
        }
        catch (err) {
            console.error("Failed to read daily note template", err, folder, template);
