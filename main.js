...
function create_else_block(ctx) {
	...
	function click_handler(...args) {
		return /*click_handler*/ ctx[11](/*date*/ ctx[16], ...args);
	}
	...
	return {
		c() {
			...
			toggle_class(td, "today", /*date*/ ctx[16].isSame(/*today*/ ctx[2]));
		...
		p(new_ctx, dirty) {
			ctx = new_ctx;
			if (dirty & /*month*/ 8 && t0_value !== (t0_value = /*dayOfMonth*/ ctx[15] + "")) set_data(t0, t0_value);
			...
			if (dirty & /*month, today*/ 12) {
				toggle_class(td, "today", /*date*/ ctx[16].isSame(/*today*/ ctx[2]));
			}

			if (dirty & /*activeFile, month*/ 9) {
				toggle_class(td, "active", /*activeFile*/ ctx[0] === /*formattedDate*/ ctx[17]);
			}
		},
		d(detaching) {
...
// (227:18) {#each Array(numDots) as _}
...
// (214:10) {#each week as { dayOfMonth, date, formattedDate, numDots }}
...
function create_each_block(ctx) {
	...
	return {
		...
		p(ctx, dirty) {
			if (dirty & /*month, today, activeFile, openOrCreateDailyNote, Array*/ 15) {
...
function create_fragment(ctx) {
	...
	return {
		...
		p(ctx, [dirty]) {
			if (dirty & /*monthName*/ 16) set_data(t1, /*monthName*/ ctx[4]);

			if (dirty & /*month, today, activeFile, openOrCreateDailyNote, Array*/ 15) {
				each_value = /*month*/ ctx[3];
				let i;
...
function instance($$self, $$props, $$invalidate) {
	...
	$$self.$$set = $$props => {
		...
		if ("dailyNoteSettings" in $$props) $$invalidate(10, dailyNoteSettings = $$props.dailyNoteSettings);
	...
	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*displayedMonth, month, dailyNoteSettings, vault*/ 1800) {
			 {
				$$invalidate(3, month = []);
				...
				for (let weekNum = 0; weekNum <= 5; weekNum++) {
					...
					for (let weekday = 1; weekday <= 7; weekday++) {
						...
						const formattedDate = date.format(dailyNoteSettings.format || DEFAULT_DATE_FORMAT);
						const baseFilename = `${formattedDate}.md`;
						const fileForDay = vault.getAbstractFileByPath(normalizedJoin(dailyNoteSettings.folder || "", baseFilename));
				...
				$$invalidate(4, monthName = displayedMonth.format("MMM YYYY"));
			}
		}
	};
...
function getDailyNoteTemplateContents(dailyNoteSettings) {
 ...
 */
async function createDailyNote(date, dailyNoteSettings) {
    const app = window.app;
    const moment = window.moment;
    const { vault } = app;
    const { folder = "" } = dailyNoteSettings;
    const format = dailyNoteSettings.format || DEFAULT_DATE_FORMAT;
    const templateContents = await getDailyNoteTemplateContents(dailyNoteSettings);
